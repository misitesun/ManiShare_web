...
export function KnowledgeGapCard({
    ...
    selected,
    onSelectedChange,
}: KnowledgeGapCardProps): ReactElement {
    const { t } = useTranslation()
    const audioSource = item.audioIcon === 'wide' ? audioWideIconSource : audioIconSource

    return (
        <article className="relative flex h-[158px] min-w-0 items-center rounded-xl bg-surface-raised px-4">
            <div
                inert={!batch}
                aria-hidden={!batch}
                className={
                    batch
                        ? 'flex w-10 shrink-0 items-center opacity-100 transition-[width,opacity,transform,translate,scale] duration-200 motion-reduce:transition-none'
                        : 'pointer-events-none flex w-0 shrink-0 -translate-x-2 items-center opacity-0 transition-[width,opacity,transform,translate,scale] duration-200 motion-reduce:transition-none'
                }
            >
                <TldSelection
                    label={`选择错题：${item.english}`}
                    checked={selected}
                    onCheckedChange={onSelectedChange}
                />
            </div>
            <div className="min-w-0 flex-1 pb-3 pt-4">
