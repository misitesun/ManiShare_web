...
import { PlayCelebration } from '../../../features/play-celebration'
...
export function FoundationPage(): ReactElement {
    ...
    return (
        <section className="mx-auto flex min-h-[calc(100dvh-4rem)] w-full max-w-[96rem] flex-col px-4 py-6 md:px-6 md:py-8 xl:min-h-[calc(100dvh-5rem)] xl:px-8 xl:py-10">
            <div>
                ...
                <div className="mt-4 grid gap-4 md:mt-6 md:grid-cols-2 xl:grid-cols-3">
                    {capabilityKeys.map((key) => (
                        ...
                        >
                            <span
                                aria-hidden="true"
                                className="block h-1.5 w-12 rounded-full bg-linear-to-r from-brand-start to-brand-end"
                            />
                            <h2 className="mt-5 text-card-title font-bold text-primary">
                                {t(`foundation.capabilities.${key}.title`)}
                            </h2>
                            <p className="mt-2 text-body text-secondary">
                                {t(`foundation.capabilities.${key}.description`)}
                            </p>
                        </article>
                    ))}
                </div>
                ...
                <aside className="mt-4 rounded-panel border border-dashed border-strong bg-surface-raised p-5 md:mt-6 md:p-6">
                    <h2 className="text-label font-bold text-primary">
                        {t('foundation.scope.title')}
                    </h2>
                    <p className="mt-2 text-body text-secondary">
                        {t('foundation.scope.description')}
                    </p>
                </aside>
            </div>
