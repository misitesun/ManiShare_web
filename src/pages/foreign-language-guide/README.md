...
- 约束：横幅和课程封面是本页面私有 Figma 导出资源；分类复用 `TldSegmentedTabs semantics="filter"` 展示受控选中和动态下划线，但暂不执行数据过滤；立即开通也暂不接业务流程。课程数据为静态分页演示数据，已加载条目始终进入同一个网格；分页观察复用 `shared/ui/tld-pagination-status`，立即开通复用 `shared/ui/tld-button`。
