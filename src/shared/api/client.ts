...
export class ApiClient {
    ...
    setAccessTokenResolver(resolver?: AccessTokenResolver): void {
    ...
    post<T>(
        ...
        return this.request<T>(path, this.withBody('POST', body, options))
    }
    put<T>(
        path: string,
        body?: RequestOptions['body'],
        ...
        return this.request<T>(path, this.withBody('PUT', body, options))
    }
    patch<T>(
        path: string,
        body?: RequestOptions['body'],
        ...
        return this.request<T>(path, this.withBody('PATCH', body, options))
    }
    ...
    async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
        const {
            ...
            headers,
            params,
            timeout = sharedConfig.api.timeout,
            signal,
            ...init
        } = options
    ...
    private buildUrl(path: string, params?: QueryParams): string {
        ...
        Object.entries(params ?? {}).forEach(([key, value]) => {
            ...
            values
                .filter(
                    (item): item is Exclude<QueryValue, null | undefined> =>
