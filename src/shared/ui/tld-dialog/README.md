...
- 响应式：`small` / `medium` / `large` 的最大宽度分别为 420 / 560 / 742px，默认 `large`；H5 保留 12px 安全边距，高度随内容自适应，不继承桌面的最小高度；取消/确认按钮始终同排等宽，间距 12px，单按钮占满操作区。平板与 PC 保留 220/280/320px 最小高度及底部居中 180px 按钮，超高内容仅在对话框内滚动。正文消费全局 body 语义字号，标题保留 Dialog 已确认的 20/22px 规格。
