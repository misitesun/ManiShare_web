...
当用户提供设计稿（Figma 链接、截图等）并要求实现页面或组件时，必须先读取可访问的设计内容，再检查仓库中相关路由、FSD 所有者、模块 README、公共入口、现有组件、状态与测试。基于这些事实，在 commentary 中向用户展示简版实施方案并明确请求确认。内部分析、通用模板或一句“准备开始实现”都不能代替这份用户可见方案。

方案固定覆盖以下五项，简单任务每项一句即可：

1. **页面区域与布局结构**：区域划分、Grid/Flex 组合、固定区与滚动区、必要的层级关系。
2. **交互行为与状态**：默认选中、切换、跳转、输入等有依据的行为；说明静态展示、真实接口接入及尚未确认的边界。
3. **复用／新增组件及理由**：写出已核实的组件名称、导入入口和当前真实消费者；区分页面私有组件与真正需要抽取的公共能力，不因视觉相似强行抽象。
4. **PC、平板、H5 适配方案**：说明断点重排、尺寸变化与溢出处理；Codex 只做 PC 人工视觉验收，平板/H5 交由用户验收。
5. **实现步骤与验收范围**：按执行顺序列出会修改的页面或公共模块、资源/状态/接口处理、文档同步，以及要运行的格式化、测试、lint、构建和 PC 交互/视觉检查；同时写明不在本次范围的内容。

方案展示后必须暂停实现，并以“请确认后我再开始修改”或同等明确表达请求用户确认。收到用户针对该方案的明确确认前，不得修改实现代码、生成/下载业务资源、安装依赖或进行会造成实现变更的 UI 操作；可以继续读取设计和代码、核对事实、回答问题或修订方案。即使需求看起来完整、用户同一条消息中已写了实现步骤，也要先结合项目现状整理出最终清单供其确认。

用户确认后，同一已确认范围内的连续实现、验证和缺陷修复不重复请求确认。若后续消息新增设计节点、页面、关键交互、公共能力、接口或验收范围，必须更新开发清单并重新等待确认。设计无法访问时应明确说明，不把猜测当作已读取的设计；不得仅凭设计稿自行增加真实接口、业务规则或第三方操作。

本规则不要求普通问答、只读诊断或已有页面的小范围修复都输出五项方案。与 Figma 工具自身的检查点或跨切面影响图重叠时合并表达，避免重复播报。
