...
接口接入或加载行为变更按[接口能力边界](./api-capability-boundaries.md#验证证据)验证缺失字段处理、请求来源与次数。使用受控请求替身覆盖缺项不触发未授权补全、列表不产生逐条详情请求、复用组件不引入无关请求或状态写入。按已确认的触发条件断言，不机械规定全页面只允许一次请求；已有分页、详情查看等合法请求保持需求约定。没有真实联调环境时明确说明证据范围。
