...
const sharedPublicSegments = new Set([
    'api',
    'config',
    'constants',
    'i18n',
...
const gsapPackages = new Set(['@gsap/react', 'gsap'])
...
const namedColorPattern =
    /\b(?:black|blue|cyan|gray|green|orange|pink|purple|red|transparent|white|yellow)\b/i
...
function getSourceContext(filePath, sourceRoot) {
    ...
    if (firstSegment === 'shared' && segments[1] !== undefined) {
        if (sharedPublicSegments.has(segments[1])) {
            return {
                layer: firstSegment,
                ownerRoot: resolve(sourceRoot, firstSegment, segments[1]),
            }
        }
        if (sharedCapabilitySegments.has(segments[1]) && segments[2] !== undefined) {
            return {
                layer: firstSegment,
                ownerRoot: resolve(sourceRoot, firstSegment, segments[1], segments[2]),
            }
        }
    }
...
function resolveLocalImport(sourceFilePath, moduleSpecifier) {
    ...
    if (extname(unresolvedPath) === '') {
        ...
        for (const extension of sourceExtensions)
            candidates.push(resolve(unresolvedPath, `index${extension}`))
    } else {
...
function isPublicRootEntry(targetFilePath, ownerRoot) {
    return (
        dirname(targetFilePath) === ownerRoot &&
        /^index\.tsx?$/.test(targetFilePath.slice(ownerRoot.length + 1))
    )
}
...
function findVisibleConstInitializer(identifier, constInitializers) {
    ...
    const candidates = (constInitializers.get(identifier.text) ?? [])
        .filter(
            (declaration) =>
                declaration.initializer.pos < identifier.pos && scopes.includes(declaration.scope),
        )
        .sort((left, right) => {
            const scopeDifference = scopes.indexOf(left.scope) - scopes.indexOf(right.scope)
            return scopeDifference === 0
                ? right.initializer.pos - left.initializer.pos
                : scopeDifference
        })
...
function analyzeClassExpression(node, constInitializers, seenInitializers = new Set()) {
    ...
    if (ts.isConditionalExpression(node)) {
        ...
        return {
            isLiteral: whenTrue.isLiteral && whenFalse.isLiteral,
            values: [...whenTrue.values, ...whenFalse.values],
        }
    }
    if (ts.isArrowFunction(node) || ts.isFunctionExpression(node)) {
        if (!ts.isBlock(node.body))
            return analyzeClassExpression(node.body, constInitializers, seenInitializers)
        const returnExpressions = []
        function collectReturns(child) {
            if (ts.isReturnStatement(child) && child.expression !== undefined)
                returnExpressions.push(child.expression)
            ts.forEachChild(child, collectReturns)
        }
        collectReturns(node.body)
        const results = returnExpressions.map((expression) =>
            analyzeClassExpression(expression, constInitializers, seenInitializers),
        )
        return {
    ...
    if (ts.isIdentifier(node)) {
        ...
        if (initializer === null || seenInitializers.has(initializer.pos))
...
function findTailwindColorViolation(classValue) {
    for (const token of classValue.split(/\s+/).filter(Boolean)) {
        ...
        if (
            ...
            colorUtilities.has(utilityName) &&
            tailwindPaletteNames.has(paletteName)
        ) {
...
function findTailwindMotionViolation(classValue) {
    const utilities = classValue
        .split(/\s+/)
        .filter(Boolean)
        .map((token) => token.slice(token.lastIndexOf(':') + 1))
    const usesTranslateUtility = utilities.some((utility) =>
        /^-?translate(?:-[xyz])?-/.test(utility),
    )
    if (!usesTranslateUtility) return null

    for (const utility of utilities) {
        const transitionMatch = utility.match(/^transition-\[([^\]]+)]$/)
        if (transitionMatch === null) continue

        const transitionProperties = transitionMatch[1]?.split(',') ?? []
        if (
            transitionProperties.includes('transform') &&
            !transitionProperties.includes('translate')
        ) {
            return `custom transition "${utility}" omits translate; Tailwind v4 translate utilities animate the translate property`
        }
    }
...
function validateImports(sourceFile, sourceRoot, errors) {
    ...
    const animationRoot = resolve(sourceRoot, 'shared', 'lib', 'animation')
    const isAnimationImplementation =
        sourceFile.fileName === animationRoot ||
        sourceFile.fileName.startsWith(`${animationRoot}${sep}`)
    ...
    function inspectModuleSpecifier(moduleSpecifierNode) {
        ...
        if (!isTestFile && (moduleSpecifier === 'electron' || nodeModules.has(moduleSpecifier))) {
            errors.push(
                `${location} renderer source must not import Node.js or Electron module "${moduleSpecifier}"`,
            )
        }

        const gsapPackage = moduleSpecifier.startsWith('gsap/') ? 'gsap' : moduleSpecifier
        if (!isAnimationImplementation && gsapPackages.has(gsapPackage)) {
            errors.push(
                `${location} import GSAP through shared/lib/animation instead of "${moduleSpecifier}"`,
            )
        }
        ...
        if (
            ...
            errors.push(
                `${location} FSD dependency violation: ${sourceContext.layer} must not import higher layer ${targetContext.layer}`,
            )
        }
        ...
        if (
            ...
            errors.push(
                `${location} same-layer slices must not import each other: ${toPosixPath(relative(sourceRoot, sourceContext.ownerRoot))} -> ${toPosixPath(relative(sourceRoot, targetContext.ownerRoot))}`,
            )
        }
        ...
        if (
            ...
            errors.push(
                `${location} cross-module imports must use the root public entry for ${toPosixPath(relative(sourceRoot, targetContext.ownerRoot))}`,
            )
        }
    }
...
function validateRendererSource(sourceFile, sourceRoot, errors) {
    ...
    const isApiImplementation =
        sourceFile.fileName === apiRoot || sourceFile.fileName.startsWith(`${apiRoot}${sep}`)
    const isTestFile = /\.test\.tsx?$/.test(sourceFile.fileName)
    const constInitializers = collectConstInitializers(sourceFile)
    const gradientTextRoot = resolve(sourceRoot, 'shared', 'ui', 'tld-gradient-text')

    function isGradientTextOuterClassNameForwarding(node) {
        if (
            sourceFile.fileName !== resolve(gradientTextRoot, 'TldGradientText.tsx') ||
    ...
    function visit(node) {
        if (!isApiImplementation && !isTestFile && ts.isCallExpression(node)) {
            ...
            if (isGlobalFetch || isWindowFetch) {
                errors.push(
                    `${getNodeLocation(sourceFile, node.expression)} use shared/api instead of calling fetch directly`,
                )
            }
        }
        ...
        if (
            ...
            if (!analysis.isLiteral && !isGradientTextOuterClassNameForwarding(node)) {
                errors.push(
                    `${getNodeLocation(sourceFile, node)} className must use complete literal strings or complete literal conditional branches`,
                )
            }
            for (const classValue of analysis.values) {
                ...
                if (violation !== null)
                    errors.push(
                        `${getNodeLocation(sourceFile, node)} className contains ${violation}; use a semantic theme utility`,
                    )
                const motionViolation = findTailwindMotionViolation(classValue)
                if (motionViolation !== null)
                    errors.push(
                        `${getNodeLocation(sourceFile, node)} className contains ${motionViolation}`,
                    )
            }
        }
        ...
        if (
            ...
            if (rawColorPattern.test(styleText) || namedColorPattern.test(styleText)) {
                errors.push(
                    `${getNodeLocation(sourceFile, node)} inline style contains a raw color; use a semantic theme utility`,
                )
            }
        }
...
function validateStyles(sourceRoot, themeStylesPath, errors) {
    const styleFiles = walkFiles(sourceRoot).filter((filePath) =>
        ['.css', '.scss', '.sass'].includes(extname(filePath)),
    )
    for (const styleFile of styleFiles) {
        if (styleFile !== themeStylesPath) {
            errors.push(
                `${toPosixPath(styleFile)} component or module stylesheet is not allowed; use literal Tailwind classes`,
            )
            continue
        }
        ...
        if (!styleContent.includes('@import "tailwindcss" source("../..");')) {
            errors.push(
                `${toPosixPath(styleFile)} must restrict Tailwind source detection to the src tree with source("../..")`,
            )
        }
        ...
        lines.forEach((line, index) => {
            ...
            errors.push(
                `${toPosixPath(styleFile)}:${index + 1}: raw colors are only allowed in --app-color-* or --app-shadow-* theme tokens`,
            )
        })
    }
}
...
export function validateSourceContracts(options = {}) {
    ...
    const themeStylesPath = resolve(
        options.themeStylesPath ?? resolve(sourceRoot, 'app', 'styles', 'index.css'),
    )
    const errors = []
    const sourceFiles = walkFiles(sourceRoot).filter((filePath) =>
        rendererExtensions.has(extname(filePath)),
    )
