...
import { createFileSystem, isAnimatedPng, optimizeStaticPngAssets } from './optimize-static-png-assets.mjs'
...
async function createFixture() {
  ...
  await writeFile(join(pageDirectory, 'FoundationPage.tsx'), "import visual from '../../shared/assets/large.png'\nexport const foundationVisual = visual\n", 'utf8')
  await writeFile(join(widgetDirectory, 'hero.css'), ".hero { background-image: url('../../shared/assets/large.png'); }\n", 'utf8')
...
test('migrates one large static PNG and synchronizes TSX and CSS references', async () => {
  ...
  try {
    ...
    assert.match(await readFile(join(rootDirectory, 'src', 'pages', 'foundation', 'FoundationPage.tsx'), 'utf8'), /large\.webp/)
    assert.match(await readFile(join(rootDirectory, 'src', 'widgets', 'hero', 'hero.css'), 'utf8'), /large\.webp/)
    ...
    assert.deepEqual(result.skipped.map((entry) => entry.reason).sort(), ['animated PNG', 'no supported static source reference', 'smaller than 512 KiB'])
  } finally {
...
test('rolls back changed source files and removes generated WebP when a commit write fails', async () => {
  ...
  await writeFile(failingSource, "body { background: url('./shared/assets/large.png'); }\n", 'utf8')
