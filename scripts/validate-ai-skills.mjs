...
const governanceDocumentPaths = ['AGENTS.md', 'docs/ai-collaboration.md', 'docs/ai-skills.md']
const requiredSkillReferences = new Map([
    ['learning-web-change-review', ['learning-web-react-engineering']],
    [
        'learning-web-cross-cutting-change',
        ['learning-web-feature-delivery', 'learning-web-react-engineering'],
    ],
    [
        'learning-web-feature-delivery',
        ['learning-web-cross-cutting-change', 'learning-web-react-engineering'],
    ],
])
...
function validateSkill(skillName) {
    ...
    if (name !== null && name[1] !== skillName)
        reportError(`${skillName}: frontmatter name must match its directory name`)
    if (name !== null && name[1].length > 64)
        reportError(`${skillName}: name must be 64 characters or fewer`)
    if (description === null || description[1].trim().length === 0)
        reportError(`${skillName}: frontmatter requires a non-empty description`)
    if (content.includes('TODO') || content.includes('<placeholder>'))
        reportError(`${skillName}: remove unfinished placeholders before committing`)
    ...
    const displayName = metadata.match(/^\s{4}display_name:\s*"([^"\n]+)"\s*$/m)
    const shortDescription = metadata.match(/^\s{4}short_description:\s*"([^"\n]+)"\s*$/m)
    const defaultPrompt = metadata.match(/^\s{4}default_prompt:\s*"([^"\n]+)"\s*$/m)
    const implicitInvocation = metadata.match(
        /^\s{4}allow_implicit_invocation:\s*(true|false)\s*$/m,
    )

    if (displayName === null)
        reportError(`${skillName}: UI metadata requires a quoted interface.display_name`)
    if (shortDescription === null)
        reportError(`${skillName}: UI metadata requires a quoted interface.short_description`)
...
if (!existsSync(skillsDirectory)) {
    ...
    for (const requiredSkillName of requiredSkillNames) {
        if (!skillDirectories.includes(requiredSkillName))
            reportError(`Missing required Skill directory: ${requiredSkillName}`)
    }
