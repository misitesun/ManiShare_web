...
function collectStaticSpecifierMatches(content) {
    ...
    const unquotedMarkupUrl =
        /\b(?:href|poster|src)\s*=\s*([^\s'"=>][^\s=>]*?\.png(?:[?#][^\s=>]*)?)/gi
...
function findReferencesForAsset(sourceFiles, assetPath, sourceDirectory, rootDirectory) {
    ...
    for (const sourceFile of sourceFiles.values()) {
        for (const match of collectStaticSpecifierMatches(sourceFile.nextContent)) {
            const referencedPath = resolveStaticAssetSpecifier(
                match.specifier,
                sourceFile.path,
                sourceDirectory,
                rootDirectory,
...
async function createMigrationPlan(rootDirectory, fileSystem) {
    ...
    const candidates = (await walkFiles(sourceDirectory, fileSystem)).filter(
        (filePath) =>
            extname(filePath).toLowerCase() === '.png' && isManagedAsset(filePath, sourceDirectory),
    )
    const skipped = []
    const migrations = []
    ...
    for (const pngPath of candidates) {
        ...
        try {
            ;[pngBuffer, pngStats] = await Promise.all([
                fileSystem.readFile(pngPath),
                fileSystem.stat(pngPath),
            ])
        } catch (error) {
        ...
        const references = findReferencesForAsset(
            sourceFiles,
            pngPath,
            sourceDirectory,
            rootDirectory,
    ...
    const changedSourceFiles = [...sourceFiles.values()].filter(
        (sourceFile) => sourceFile.nextContent !== sourceFile.originalContent,
    )
...
async function cleanTemporaryFiles(temporaryPaths, fileSystem) {
    await Promise.all(
        temporaryPaths.map((filePath) =>
            fileSystem.rm(filePath, { force: true }).catch(() => undefined),
        ),
    )
}

async function verifyNoRemainingStaticReferences(
    migrations,
    sourceDirectory,
    rootDirectory,
    fileSystem,
...
async function commitMigrationPlan(plan, fileSystem) {
    ...
    const activeMigrations = plan.migrations.filter((migration) =>
        migration.references.some((reference) => activeSourcePaths.has(reference.sourcePath)),
    )
    ...
    try {
        ...
        await verifyNoRemainingStaticReferences(
            activeMigrations,
        ...
        await Promise.all(
            committedSourceFiles.map((sourceFile) =>
                fileSystem
                    .writeFile(sourceFile.path, sourceFile.originalContent, 'utf8')
                    .catch(() => undefined),
            ),
        )
        await Promise.all(
            committedWebpFiles.map((migration) =>
                fileSystem.rm(migration.webpPath, { force: true }).catch(() => undefined),
            ),
        )
        await Promise.all(
            deletedPngFiles.map((migration) =>
                fileSystem.writeFile(migration.pngPath, migration.pngBuffer).catch(() => undefined),
            ),
        )
...
export async function optimizeStaticPngAssets(options = {}) {
    ...
    for (const migration of migrated) {
        logger.log(
            `[assets:optimize] ${relative(rootDirectory, migration.pngPath)} -> ${relative(rootDirectory, migration.webpPath)} (${migration.references.length} references)`,
        )
    }

    for (const skipped of plan.skipped) {
        logger.log(
            `[assets:optimize] skipped ${relative(rootDirectory, skipped.path)}: ${skipped.reason}`,
        )
    }
